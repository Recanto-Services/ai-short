...
def convert_to_vertical(input_video_path, output_video_path):
    try:
        # Carregar o vídeo
        clip = VideoFileClip(input_video_path)

        # Calcular a nova altura e largura para a proporção 9:16
        original_width, original_height = clip.size
        vertical_height = original_height
        vertical_width = int(vertical_height * 9 / 16)

        # Redimensionar o vídeo para a proporção 9:16
        if original_width < vertical_width:
            print("Error: Original video width is less than the desired vertical width.")
            return

        # Cortar o vídeo para a proporção 9:16
        x_center = original_width // 2
        x_start = x_center - (vertical_width // 2)
        x_end = x_center + (vertical_width // 2)
        vertical_clip = clip.crop(x1=x_start, x2=x_end)

        # Salvar o vídeo redimensionado
        vertical_clip.write_videofile(output_video_path, codec='libx264')
        print(f"Vertical video saved successfully as {output_video_path}")

    except Exception as e:
        print(f"Error converting video to vertical: {str(e)}")
...
def crop_to_vertical(input_video_path, output_video_path):
    ...
    cap.release()
    out.release()
    print("Cropping complete. The video has been saved to",
